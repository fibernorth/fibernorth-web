 ...
 */
async function plan(db: Firestore, params: ImportParams, batchId: string, now: string): Promise<Plan> {
...
export async function applyImport(
  ...
  const p = await plan(db, params, runId, now);
  ...
  await inChunks(run.createdIds || [], async (id) => {
    ...
    const r = await db.runTransaction(async (tx) => {
      ...
      tx.set(db.collection("trash").doc(`leads__${id}`), {
        ...
        deletedAt: now,
        deletedBy: opts.by,
        reason: `Undo of import batch ${runId}`,
      });
      tx.delete(ref);

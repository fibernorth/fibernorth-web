...
async function mirrorToLead(
  ...
  try {
    ...
    if (leadSnap?.exists) {
      ...
      // `by` only when a person did it (the put-back); callbacks and pulls are Bore-ON.
      const entry: LeadActivity = { ts: now, type: "system", text, ...(by ? { by } : {}) };
...
export async function restorePreviousBoreOnPrices(
  ...
  await mirrorToLead(db, quoteId, r.leadId, `Bore-ON prices put back: ${money(r.was)} → ${money(r.total)}`, now, {}, by);

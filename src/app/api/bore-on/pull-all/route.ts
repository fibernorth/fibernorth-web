...
import { OWNER_ONLY_MESSAGE } from "@/lib/admin-allowlist";
...
export async function POST(request: Request) {
  ...
  // Previews are open to staff; applying a bulk re-price is owner-only.
  if (apply && !auth.owner) return NextResponse.json({ error: OWNER_ONLY_MESSAGE }, { status: 403 });
